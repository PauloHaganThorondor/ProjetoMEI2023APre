...
def joaoprofessor():
    popup_layout = [
            [sg.Text('''
O professor do curso de Programação é o professor João Ornellas, 
que também é professor de Robótica. Para saber mais acesse no QR Code.
                     ''',font=(
                'Verdana', 12,'italic','bold'), text_color='white',)],
            [sg.Canvas(background_color=None, size=(180, 10)),sg.Image(filename='QRCodeJoao.png',pad=(10))],
            [sg.Canvas(background_color=None, size=(235, 10)),sg.Button('Fechar Pop-up', size=(20, 1), pad=(20, (0, 20)),
                       border_width=4, button_color=('white', '#4682B4'))]
        ]
    popup_window = sg.Window('Imagem em Pop-up', popup_layout,no_titlebar=True, modal=True)

    while True:
        popup_event, popup_values = popup_window.read()

        if popup_event in (sg.WINDOW_CLOSED, 'Fechar Pop-up'):
            break
...
def telaapresentacao():
    ...
    layout = [
        ...
        [sg.Frame(" Projeto da Turma A de Programação ", font=('Tahoma', 18), layout=[
            [sg.Canvas(background_color=None, size=(130, 10)), sg.Frame("", layout=[
...
def tela3(RendaBruta, DespesaRenda):
    ...
    layout = [
        [sg.Frame(" Tela de Declaração de Ocupação do MEI ", font=('Helvetica', 14), layout=[
            ...
            [sg.Canvas(background_color=None, size=(200, 10)), sg.Frame("", layout=[
                [sg.Image(gifimagem3, key='-IMAGE-', enable_events=True)]], relief=sg.RELIEF_RAISED, border_width=4, pad=(10))],
            [sg.Button('Prosseguir ➡', pad=(15), border_width=4)]
        ], relief=sg.RELIEF_SUNKEN, border_width=5,pad=(10))]
    ]
...
'''
    layout = [
        [sg.Frame(" Tela de Demonstração da Renda Tributável ", font=('Helvetica', 14), layout=[
             [sg.Frame("",layout=[
                [sg.Text('Você Chegou ao Fim da Simulação, Parabéns', background_color='#008080', text_color='#C0C0C0',font=(
                'Helvetica', 19,'italic'))] ],relief=sg.RELIEF_SUNKEN, border_width=3,pad=(10))],
